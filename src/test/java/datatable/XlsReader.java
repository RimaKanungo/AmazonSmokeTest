...
public class XlsReader 
{
	...
	/*   
	 ...
	 3) Workbook.getWorkbook(<xls workbook file>) -> Load the xls workbook file
	...
	/*   
	 public void writetoTheCell(String sheetName, int colIndex, int rowIndex, String text) method specification :-
	 1)  sheetName -> name of the excel sheet. 
	 2)  colIndex, rowIndex -> column name and row no. respectively within the sheet. 
	 3)  Workbook -> Refer to the whole xls file.
	 4)  Sheet -> Refer to an individual sheet within that workbook.
	 5)  createWorkbook -> Creates a workbook
	 6)  getSheet() -> Used to hold the control of a particular sheet
	 7)  

	 */
	
	public void writetoTheCell(String sheetName, int colIndex, int rowIndex, String text) throws BiffException, IOException, RowsExceededException, WriteException
	{
	...
	Label label = new Label(colIndex, rowIndex, text);
	sheet.addCell(label);
	copy.write();
	copy.close();
